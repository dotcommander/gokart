...
func configureNewCommandFlags(cmd *cobra.Command) {
	...
	flags.Bool(newFlagLocal, false, "Use local-only config (default)")
	flags.Bool(newFlagGlobal, false, "Enable global config")
	...
	flags.Bool(newFlagNoManifest, false, "Skip writing .gokart-manifest.json for managed scaffolds")
