...
func TestApplyWritesManifestWithFileProvenance(t *testing.T) {
	...
	targetDir := t.TempDir()
	const generatorVersion = "v-test"
	result, err := Apply(fsys, "templates/basic", targetDir, TemplateData{}, ApplyOptions{ExistingFilePolicy: ExistingFilePolicyOverwrite, GeneratorVersion: generatorVersion})
	if err != nil {
	...
	if manifest.GeneratorVersion != generatorVersion {
		t.Fatalf("unexpected manifest generator version: got %q, want %q", manifest.GeneratorVersion, generatorVersion)
	}
