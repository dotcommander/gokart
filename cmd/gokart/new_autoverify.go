...
func resolveAutoVerify(req newRequest, explicitVerify, noVerify bool, warnings *[]string) bool {
	if explicitVerify {
		return true
	}
	if noVerify {
...
func autoVerifyEnvDisabled() bool {
	raw, ok := os.LookupEnv("GOKART_AUTO_VERIFY")
	if !ok {
		return false
	}
